...
public class PoiUtil {
    ...
    public static void importCsvGoodDataFile(InputStream fileInputStream) {
        try {
            Reader reader = new InputStreamReader(fileInputStream);
            CsvReader csvReader = new CsvReader(reader);
            // 读表头
            ...
            while (csvReader.readRecord()) {
                saveGoodEntity(dataColumnList, csvReader);
            }
        } catch (Exception e) {
            System.out.println(e);
            throw new ServiceException(ResultEnum.FILE_ERROR);
    ...
    public static void saveGoodEntity(List<String> dataColumnList, CsvReader csvReader) throws Exception {
        Good good = new Good();
        for (int i = 0; i < dataColumnList.size(); i++) {
            String column = dataColumnList.get(i);
            Field field = good.getClass().getDeclaredField(column);
            field.setAccessible(true);
            field.set(good, converAttributeValue(field.getType().getName(), csvReader.get(column)));
        }
